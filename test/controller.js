...
contract('Controller test', ([alice, bob, carol, dev]) => {
    ...
    before(async () => {
        ...
        await this.dope.setFactory(this.factory.address, {from: dev});
    ...
    beforeEach(async () => {
        this.lock = await Lock.new(this.dope.address, {from: dev});
        await this.dope.setLock(this.lock.address, {from: dev});

        this.controller = await Controller.new(this.dope.address, {from: dev});
        await this.dope.setController(this.controller.address, {from: dev});
    });

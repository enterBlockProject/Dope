...
contract('Distributor test', ([alice, bob, carol, dev, minter]) => {
    ...
    context('With BEP/LP token added to the field', () => {
        ...
        it('should give out dopes only after farming time', async () => {
            this.distributor = await Distributor.new(
                ...
                '1000',
                { from: alice });
            await this.dope.addMinter(this.distributor.address, {from: dev});
            ...
            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '0');
            ...
            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '100');
            await time.advanceBlockTo('104');
            await this.distributor.claim(0, { from: bob }); // block 105
            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '500');

            assert.equal((await this.dope.totalSupply()).valueOf(), '500');
        });

        it('should not distribute dopes if no one deposit', async () => {
            this.distributor = await Distributor.new(
                this.dope.address,
                '100',
                '200',
            ...
            assert.equal((await this.dope.totalSupply()).valueOf(), '0');
            ...
            assert.equal((await this.dope.totalSupply()).valueOf(), '0');
            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '0');
            assert.equal((await this.dope.balanceOf(dev)).valueOf(), '0');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '990');
            await time.advanceBlockTo('219');
            await this.distributor.withdraw(0, '10', { from: bob }); // block 220

            assert.equal((await this.dope.totalSupply()).valueOf(), '1000');

            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '1000');

            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '1000');
        });

        it('should distribute dopes properly for each staker', async () => {
            this.distributor = await Distributor.new(
                this.dope.address,
                '100',
                '300',
            ...
            assert.equal((await this.dope.totalSupply()).valueOf(), '566');

            assert.equal((await this.dope.balanceOf(alice)).valueOf(), '566');
            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '0');
            assert.equal((await this.dope.balanceOf(carol)).valueOf(), '0');

            await time.advanceBlockTo('329')
            await this.distributor.withdraw(0, '5', { from: bob });

            assert.equal((await this.dope.totalSupply()).valueOf(), '1185');

            assert.equal((await this.dope.balanceOf(alice)).valueOf(), '566');
            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '619');
            assert.equal((await this.dope.balanceOf(carol)).valueOf(), '0');
            ...
            assert.equal((await this.dope.totalSupply()).valueOf(), '4999');

            assert.equal((await this.dope.balanceOf(alice)).valueOf(), '1159');
            assert.equal((await this.dope.balanceOf(bob)).valueOf(), '1183');
            assert.equal((await this.dope.balanceOf(carol)).valueOf(), '2657');
        ...
        it('should give proper dopes allocation to each pool', async () => {
            this.distributor = await Distributor.new(
                this.dope.address,
                '100',
                '400',
        ...
        it('should stop giving distributors after the period ends', async () => {
            ...
            assert.equal((await this.dope.balanceOf(alice)).valueOf(), '1000');
        });
    });
});